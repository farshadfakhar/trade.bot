...
async function getBalance(cur) {
    console.log('GETING', cur, 'BALANCE')
    const data = await nobiPost(
...
async function emergencySell() {
    try {
        ...
        console.log("⚠️ EMERGENCY SELL →", btc, "@", price);
        sendTelegram(`⚠️ EMERGENCY SELL TRIGGERED → ${btc} @ ${price}`);

        await placeOrder("sell", price, btc);
...
async function strategyLoop() {
    ...
    TRADE_AMOUNT = Math.floor(TRADE_USDT / price * 1e6) / 1e6;

    last3.push(price);
    if (last3.length > 3) last3.shift();

    console.log("\nSTATE:", STATE, "| PRICE:", price, "| lastPriceStored:", lastPriceStored, " | TRADE AMOUNT: ",TRADE_AMOUNT);
...
async function getCurrentWallet() {
    ...
    const btc = await getBalance("btc");
    console.log(`WALLET   USDT: ${usdt} | RIAL: ${rls} | BTC: ${btc}`)
    sendTelegram(`WALLET   USDT: ${usdt} | RIAL: ${rls} | BTC: ${btc}`)
}
