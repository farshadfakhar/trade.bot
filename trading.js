...
async function logicLoop() {
    ...
    if (analysis.signal === 'buy') {
        if (!data.lastPrice) {
            const place = await placeOrder('buy', bestBuy);
            data.lastPrice = bestBuy;
            data.buys += 1;
            data.logs.push({ type: 'buy', bestBuy })
            sendTelegram(`Buy executed at price: ${bestBuy}`);
            console.log(`Buy executed at price: ${bestBuy}`);
    ...
    if (analysis.signal === 'sell') {
        ...
        if (bestSell >= target) {
            ...
            if (place) {
                sendTelegram(`Sell executed at ${bestSell} | amount=${balanceBTC}`);
                console.log(`Sell executed at ${bestSell}`);
                data.lastPrice = 0;
                data.buys = 0;
                data.logs.push({ type: 'sell', price: bestSell })
            }

        } else {
            console.log(`Sell signal detected, but price < target. Now=${bestSell}, Target=${target}`);
            data.state = 'HOLD'
        }
    }
